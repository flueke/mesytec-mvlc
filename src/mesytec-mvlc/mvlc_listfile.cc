...
Preamble read_preamble(ReadHandle &rh, const size_t preambleMaxSize)
{
    ...
    while (true)
    {
        ...
        {
            // Note: with library generated listfiles there is a BeginRun
            // section which is handled below so this code path is never
            // executed. Listfiles generated by mvme-1.0.1 do not have the
            // BeginRun section so the above frameHeader read will always read
            // into the beginning of actual readout data. This is not a problem
            // as we seek to directly past the magic bytes before returning.
            break;
        }
