...
#include "mvlc_trigger_io_port.h"
...
{
    return minimize(lut.lutContents[0]) | minimize(lut.lutContents[1]) |
...
make_l2_input_choices()
{
    ...
    {
        // Common to all inputs: can connect to Level0 up to but excluding
        // sysclock
        ...
        // All L2 LUTs
        for (unsigned i = 0; i < LUT_DynamicInputCount; i++)
            result[unit].lutChoices[i].push_back(
                UnitAddress{0, trigger_io::Level0::DAQStartOffset});
...
std::vector<std::vector<UnitAddressVector>> make_l3_input_choices()
{
    ...
    for (size_t i = 0; i < trigger_io::Level3::CountersCount; i++)
    {
        ...
        // L0 units. Since FW0016 both inputs can also connect to the new
        // L2.LUT2. The latch input also has a special "not-connected" value
        // which is one past the last valid connection value but this value is
        // not stored in here for now.
...
Level3::Level3()
{
    ...
    for (unsigned unit = Level3::CountersOffset;
         unit < Level3::CountersOffset + Level3::CountersCount; unit++)
    {
...
std::string lookup_name(const TriggerIO &cfg, const UnitAddress &addr)
{
    try
    {
        switch (addr[0])
        {
        case 0:
            return cfg.l0.unitNames[addr[1]];
        ...
        case 3:
            return cfg.l3.unitNames[addr[1]];
        }
    }
...
std::string lookup_default_name(const TriggerIO &cfg, const UnitAddress &addr)
{
    switch (addr[0])
    {
    case 0:
