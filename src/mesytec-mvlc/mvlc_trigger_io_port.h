...
#ifndef CA5CAB66_9998_4CFA_9112_29477B1C5541
#define CA5CAB66_9998_4CFA_9112_29477B1C5541
...
struct MESYTEC_MVLC_EXPORT StackBusy
{
    u16 stackIndex;
};
...
struct MESYTEC_MVLC_EXPORT LUT
{
    ...
    // Same as above but for the strobe output which is only used in the
    // simulation code.
    static const u16 StrobeGGOutput = OutputBits;

    // Holds the output state for all 64 input combinations. One of the logic
    // LUTs is made up for three of these mappings, one for each output bit.
...
// bits cleared.
MESYTEC_MVLC_EXPORT std::bitset<LUT::InputBits> minimize(const LUT &lut);

struct MESYTEC_MVLC_EXPORT StackStart
{
    bool activate;
    u8 stackIndex;
    u16 delay_ns;
...
struct MESYTEC_MVLC_EXPORT MasterTrigger
{
    bool activate;
    // Note: the immediate flag is used to directly exec the stack which can be
    // useful for testing. The value is not currently stored in here.
...
struct MESYTEC_MVLC_EXPORT Counter
{
...
struct MESYTEC_MVLC_EXPORT IRQ_Util
{
...
struct MESYTEC_MVLC_EXPORT TriggerResource
{
...
struct MESYTEC_MVLC_EXPORT UnitConnection
{
...
static const std::string UnitNotAvailable = "N/A";

struct MESYTEC_MVLC_EXPORT Level0
{
    ...
    static const int OutputCount =
        UtilityUnitCount + NIM_IO_Count + NIM_to_IRQ_Gap + IRQ_Inputs_Count;

    static const std::array<std::string, OutputCount> DefaultUnitNames;
...
struct MESYTEC_MVLC_EXPORT Level1
{
...
struct MESYTEC_MVLC_EXPORT Level2
{
...
struct MESYTEC_MVLC_EXPORT Level3
{
...
struct MESYTEC_MVLC_EXPORT TriggerIO
{
    Level0 l0;
    Level1 l1;
    Level2 l2;
