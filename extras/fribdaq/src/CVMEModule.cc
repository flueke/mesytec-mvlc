...
CVMEModule::CompileItem(CTCLObject& op) {
  ...
  if (std::string(definition[0]) == "r") {
    ...
    // Read block - must be 4 items:
    if (definition.size() != 4) {
      std::stringstream smsg;
      smsg << std::string(op) << " must be a 4 element list for read block operations";
      std::string msg(smsg.str());
      throw std::invalid_argument(msg);
    }
    int amod = definition[1];
    ...
    result.s_width = mesytec::mvlc::VMEDataWidth::D32;  // unused.
  } else if (std::string(definition[0]) == "rf") {
    // Read fifo - must be 4 items:
    if (definition.size() != 4) {
      std::stringstream smsg;
      smsg << std::string(op) << " must be a 4 element list for read fifo operations";
      std::string msg(smsg.str());
      throw std::invalid_argument(msg);
    }
    int amod = definition[1];
    uint32_t adr =  int(definition[2]);
    uint32_t count = int(definition[3]);
    mesytec::mvlc::VMEDataWidth wid = mesytec::mvlc::VMEDataWidth::D32;  // unused.
